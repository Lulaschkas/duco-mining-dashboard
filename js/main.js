...
function networkgauges(chart_net, time) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == "4" && this.status == 200) { //Check if the website is not loading anymore and webserver returns status code 200#
            ...
            let roundedhashrate = Math.round(hashrate2[0] * 100) / 100; //Round hashrate
...
function updateuserbalance(user, graph, time) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == "4" && this.status == 200) { //Check if the website is not loading anymore and webserver returns status code 200
            userbalance = parseFloat(validate(xmlhttp.response["result"]["balance"].toString())); //Update the global let
            let rounded = Math.round(userbalance * 100) / 100;
...
function problems(miners, geseffavr, geseffpc) { //Check for possible problems while mining
    ...
    miners.forEach( //For each mining device
        function(element) {
            let fix = "<a href='fixes.html'>How to fix</a></p>"
            if ((element[0].includes("ESP8266") || element[0].includes("ESP")) && !element[0].includes("ESP32")) {
                let info = "<p id='problems'><b>ESP8266 with Rigname:</b> " + element[7] + " <b>and Softwarename:</b> " + element[0] + " <b>with Hashrate:</b> " + element[1] + "H/s ";
            ...
            if (element[0].includes("ESP32")) { //checking for ESP32
                let info = "<p id='problems'><b>ESP32 with Rigname:</b> " + element[7] + " <b>and Softwarename:</b> " + element[0] + " <b>with Hashrate:</b> " + element[1] + "H/s ";
            ...
            if (element[0].includes("AVR")) {
                let info = "<p id='problems'><b>Arduino with Rigname:</b> " + element[7] + " <b>and Softwarename:</b> " + element[0] + " <b>with Hashrate:</b> " + element[1] + "H/s ";
            ...
            if (element[0].includes("PC Miner") && element[5].includes("XXHASH")) {
                let info = "<p id='problems'><b>PC (XXHASH) with Rigname: </b>" + element[7] + " <b>and Softwarename:</b> " + element[0] + " <b>with Hashrate:</b> " + element[1] + "H/s ";
            ...
            if (element[0].includes("PC Miner") && element[5].includes("DUCO-S1")) {
                let info = "<p id='problems'><b>PC (DUCO-S1) with Rigname:</b> " + element[7] + " <b>and Softwarename: </b>" + element[0] + " <b>with Hashrate:</b> " + element[1] + "H/s ";
                if (element[2] > 3) { //Only check if more than 3 shares got submitted
                    if (element[1] < 30000) { //Too low Hashrate
                        document.getElementById("problemt").innerHTML += info + " is mining too slow. " + fix;
                        problems++;
...
function minerdata(username, chart_hash, chart_con, time) { //This function is sorting all miners for the given username
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == "4" && this.status == 200) { //Check if the website is not loading anymore and webserver returns status code 200
            ...
            let max = (esp[0] * 13000 + avr[0] * 180 + pc[0] * 500000) / 1000000; //Calculate the maximum possible hashrate for the user based on Kolka V4 
...
function calculdaily(newb, oldb) {
    ...
    //Calculate per day
    let dayduco = ducomadein * 5760; //86400 seconds daily / 15s = 5760
    ...
    let secondssincestart = (Date.now() - start) / 1000; //MIlliseconds since sart of the page
...
let inv1 = 0; // Global let for Interval - callback checker function
...
function btnfunc() {
    let username = validate(document.getElementById("inputt").value); //Get the username
...
function checker(username) { //Callback function
    ...
    if (callback >= 3) { //If every ping was successfull
        ...
        let chart = makegraph();
        let chart_net = makenetchart();
        let chart_hash = makehashratechart();
        let chart_con = makeconchart();
        ...
        if (this.readyState == "4") { //Check if the website is not loading anymore and webserver returns status code 200
            if (this.status == 200) {
                ...
                if (balanc["result"] != undefined) {
                    ...
                    document.getElementById("usernamei").innerHTML = validate(balanc["result"]["username"]) + "'s balance";
